...
object eu141 {
  ...
  def run3() = {
    (1L to 1000L).foreach { p =>
      (isProgressive(p * p))
    }
  }

  def isProgressive3(n: Long): Boolean = {
    val sqrt = Math.sqrt(n).toLong+1
    (1L to sqrt).exists(d => isProgressive3(n, d))
  }

  def isProgressive3(n: Long, d: Long): Boolean = {
    val q = n/d
    val r = n%d
    val ordered = Seq(d,r,q).sorted
    val yes = ordered(0)*ordered(2) == ordered(1)*ordered(1)
